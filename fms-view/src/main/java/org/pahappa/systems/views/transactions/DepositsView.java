...
public class DepositsView extends PaginatedTableView<Deposit, DepositsView, DepositsView>{

	/**
	 *
	 */
	private List<Deposit> filteredTemplate;
	...
	@PostConstruct
	public void init() {
		this.depositService = ApplicationContextProvider.getApplicationContext().getBean(DepositService.class);
		this.sortFields = Arrays.asList(new SortField("First Name Asc", "user.firstName", false),
				...
				new SortField("Credit Date Asc", "depositDate", false),
				new SortField("Credit Date Desc", "depositDate", true));
	...
	public void reloadFilterReset(){
		this.setTotalRecords(this.depositService.countSearchedDeposits(this.searchTerm, this.startDate, this.endDate, this.sortField));
		try {
	...
	public void reloadFromDB(int offset, int limit, Map<String, Object> arg2) throws Exception {
		super.setDataModels(depositService.getUserDeposits(this.searchTerm, this.startDate, this.endDate, this.selectedSortField, limit,offset));
	}
	...
	/**
	 * @return the filteredTemplate
	 */
	public List<Deposit> getFilteredTemplate() {
		return filteredTemplate;
	}

	/**
	 * @param filteredTemplate the filteredTemplate to set
	 */
	public void setFilteredTemplate(List<Deposit> filteredTemplate) {
