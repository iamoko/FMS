...
//          System.out.println(cal.getStartDate()+" "+cal.getEndDate());
    ...
    public void employeeRecord(User users) {
        this.theEmployee = users;
        for (NonWorkingDays cal : this.nonWorkingDaysService.getCalendar(users)) {
...
//          System.out.println(cal.getStartDate()+" "+cal.getEndDate());
    ...
    public User getTheEmployee() {
        return theEmployee;
    }

    public void setTheEmployee(User theEmployee) {
    ...
    public void addEvent() {
        ...
        for (Date date : DateUtils.getDates(convertToDate(event.getStartDate()), convertToDate(event.getEndDate()), 1)) {
            ...
            if (!failed) {
                ...
                NonWorkingDays nonWorkingDays = new NonWorkingDays();
                nonWorkingDays.setUser(this.theEmployee);
                nonWorkingDays.setStartDate(convertToDate(event.getStartDate()));
                nonWorkingDays.setEndDate(convertToDate(event.getEndDate()));
                this.nonWorkingDaysService.setDate(nonWorkingDays);
        ...
        FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Action Successful", calendar_title + " Non working Day Set where necessary");
        PrimeFaces.current().dialog().showMessageDynamic(message);


    }

    public String getBalance(User employee){
        NumberFormat myFormat = NumberFormat.getInstance();
        return myFormat.format((int)this.accountService.getAccountByUser(employee).getOutstandingBalance());
    }

    public String getEntitled(User employee){
        NumberFormat myFormat = NumberFormat.getInstance();
        return myFormat.format((int)this.accountService.getAccountByUser(employee).getEntitledAllowance());
    }
    public String getPhone(User employee){
